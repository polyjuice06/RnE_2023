...
to setup-patches
  ask patches [
    let randchap random-float 1.0
  let sum-ratio ratio-random + ratio-imitate
  (ifelse randchap < (ratio-random / sum-ratio) [
    set rps-strategy "random-play"
    set pcolor red
  ]
  randchap < ((ratio-random + ratio-imitate) / sum-ratio) [
      set rps-strategy "imitate-play"
      set pcolor blue
    ])
  ]
end
...
to link-patches
  let patch-num count patches
  let whole_list ( shuffle ( range patch-num ) )
  let patchnum-list1 ( sublist whole_list 0 ( patch-num / 2 ) )
  let patchnum-list2 ( sublist whole_list ( patch-num / 2 ) patch-num )
end
...
to test
  let whole_list ( shuffle ( range 60 ) )
  let list_split_1 ( sublist whole_list 0 30 )
  let list_split_2 ( sublist whole_list 30 60 )
  show whole_list
  show list_split_1
  show list_split_2
  (foreach list_split_1 list_split_2
    [ [a b] -> ask turtle a [ create-link-with turtle b ] ])
end
