...
turtles-own [ energy
  mutant ]  ;; agents own energy, 감염에 대한 요소 추가
...
to setup
  ...
  create-sheep number-of-sheep [  ;; create the initial sheep
    ...
    set mutant random 10 ;; 감염 여부 설정 , 색으로 표시
    ifelse mutant = 0
    [set color orange]
    [set color white]
  ]
...
to eat-sheep
  if any? sheep-here [  ;; if there are sheep here then eat one
    ...
    let left-mutant 0
    ask target [
      set left-energy energy * 0.007 + energy-gain-from-sheep * 0.5 ;; 죽은 것으로부터 얻는 에너지 값
      ifelse mutant = 0
      [set left-mutant 1]
      [set left-mutant 0]
      die
    ...
    ifelse left-mutant = 1
    [let luck random 10
    ifelse luck <= (3 + (random 2))
      [die]
      [set mutant 0]] ;; 감염된 것을 먹으면 30~50%의 확률로 감염되거나 70~50%의 확률로 죽거나
    [set energy energy + left-energy]
  ]
end
...
to eat-wolf
  if any? wolves-here[
    ...
    let left-mutant 0
    ask target[
      set left-energy energy * 0.012 + energy-gain-from-wolf * 0.5 ;; 죽은 것으로부터 얻는 에너지 값
      ifelse mutant = 0
      [set left-mutant 1]
      [set left-mutant 0]
      die
...
to reproduce
  if energy > 200 [
   ...
   set mutant random 10 ;;새로 태어난 것이 감염되어 있도록 세팅
   ifelse mutant = 0
      [ifelse breed = sheep
      [set color orange]
        [ifelse breed = wolves
      ...
      [ifelse breed = sheep
        ...
        [ifelse breed = wolves
        [set color brown]
        [set color red]] ] ;; to the new agent
  ]
  ]
end
...
to move
  forward 1
  ifelse mutant = 0
  [set energy energy - movement-cost *  (1.2 + random-float 1)] ;; 감염되어서 더 많은 에너지를 소모
  [set energy energy - movement-cost *  (0.5 + random-float 1)];; reduce the energy by the cost of movement
end
...
"grass" 1.0 0 -10899396 true "" "plot sum [ grass-amount ] of patches / 50"
"wolves" 1.0 0 -6459832 true "" "plot count wolves * 10"
"bears" 1.0 0 -7500403 true "" "plot count bears * 10"
...
"" ""
PENS
"msheep" 1.0 0 -16777216 true "" "plot count sheep with [mutant = 0]"
"mwolf" 1.0 0 -7500403 true "" "plot count wolves with [mutant = 0]"
"mbear" 1.0 0 -2674135 true "" "plot count bears with [mutant = 0]"
