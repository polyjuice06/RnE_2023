...
to setup
  ...
  create-wolves number-of-wolves [  ;; create the initial wolves
    ...
    set shape "wolf"
    set fst-size 2
    set size fst-size ;; increase their size so they are a little easier to see
    set energy 100  ;; set the initial energy to 100
  ]
  reset-ticks
end
...
to go
  ...
  let will-bumb random-float 1
  if will-bumb < ( bumb-percent / 100 ) [
    bumb-drop
  ]
  ask wolves [  ;; ask both wolves and sheep
  ...
  ask sheep [  ;; ask both wolves and sheep
    wiggle  ;; first turn a little bit
    move  ;; then step forward
    check-if-dead  ;; check to see if agent should die
    eat            ;; sheep eat grass, wolves eat sheep
...
to eat-sheep
  if any? sheep-here [  ;; if there are sheep here then eat one
    ...
    set energy energy + target_energy * 0.07
  ]
end
...
;; change size in proportion to energy
to change-size
  set size fst-size * ( energy / 100 )
end
