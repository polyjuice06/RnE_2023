...
to setup
  ...
  create-tigers number-of-tigers [  ;; create the initial tigers
    setxy random-xcor random-ycor
    set color red
    set shape "wolf"
    set size 3 ;; increase their size so they are a little easier to see
    set energy 100  ;; set the initial energy to 100
  ]
  if Human [
    create-humans 1 [
...
to eat-wolves
  if any? wolves-here [  ;; if there are wolves here then eat one
    let target one-of wolves-here
    ask target [
      die
    ]
    ;; increase the energy by the parameter setting
    set energy energy + energy-gain-from-wolves
  ]
end

to eat-humans
  if ( any? humans-here and ( random 3 = 1 ) ) [  ;; if there are human here then eat one
...
to eat-tigers
  if any? tigers-here [  ;; if there are tigers here then eat one
    let target one-of tigers-here
    ask target [
      die
    ]
    ;; increase the energy by the parameter setting
...
to reproduce
  if breed = sheep [
    if energy > 100 [
      set energy energy - 90  ;; reproduction transfers energy
      hatch 1 [ set energy 90 ] ;; to the new agent
    ]
  ]
  if breed = wolves [
    if energy > 150 [
  ...
  if breed = tigers [
    if energy > 200 [
      set energy energy - 200  ;; reproduction transfers energy
      hatch 1 [ set energy 150 ] ;; to the new agent
    ]
  ]
  ...
  if breed = humans [
    if energy > 150 [
      set energy energy - 100  ;; reproduction transfers energy
      hatch 1 [ set energy 20 ] ;; to the new agent
      if ( random 100 = 1 ) [ hatch 1 [ set energy 20 ] ] ;; twins
    ]
  ]
end
...
to eat
  ...
  if breed = humans [
    eat-grass
    eat-sheep
    if ( random 2 = 1) [ eat-wolves ]
    if ( random 4 = 1) [ eat-tigers ]
  ]
end
...
to my-update-plots
  ...
  plot count wolves * 20 ;; scaling factor so plot looks nice

  set-current-plot-pen "tigers"
  plot count tigers * 50 ;; scaling factor so plot looks nice

  set-current-plot-pen "grass"
  plot sum [ grass-amount ] of patches / 5 ;; scaling factor so plot looks nice


  set-current-plot-pen "humans"
  plot count humans * 50 ;; scaling factor so plot looks nice
end
...
"tigers" 1.0 0 -2139308 true "" ""
"humans" 1.0 0 -13345367 true "" ""
